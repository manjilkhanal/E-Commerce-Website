...
class ProductController {
    ...
    featured = async(req, res, next) =>{
        try{
            const products = await Product.find({status:true, featured:true}).sort({'createdAt': 'desc'}).exec()
            res.json(products)
        }catch(err){
            showError(err, next)
        }
    ...
    top = async (req, res, next) =>{
        try{
            const products = await Product.aggregate([
                { $match: {status: true}},
                {$lookup: {from: 'orderdetails', localField: '_id',
                foreignField: 'product_id',as: 'order_count'}},
    ...
    similar = async (req, res, next) =>{
        try{
            ...
            const products = await Product.find({status: true,
            category_id: product.category_id, _id: {$ne: product._id}})
            .exec()
            res.json(products)
        }catch(err){
            showError(err, next)
    ...
    byCategory = async (req, res, next) =>{
        try{
            ...
            const products = await Product.find({status: true, category_id:
            req.params.id}).exec()
            res.json(products)
        }catch(err){
            showError(err, next)
    ...
    byBrand = async (req, res, next) =>{
        try{
            ...
            const products = await Product.find({status: true, brand_id:
            req.params.id}).exec()
            res.json(products)
        }catch(err){
            showError(err, next)
    ...
    byId = async (req, res, next) =>{
        try{
            const products = await Product.aggregate([
                {$match:{status:true, _id: new mongoose.Types.ObjectId
                (req.params.id)}},
                {$lookup: {from: 'categories',localField:'category_id',
            foreignField: '_id', as:'category'}},
            {$lookup: {from: 'brands',localField:'brand_id',
        ...
        const reviews = await Review.aggregate([
            {$match:{product_id: new mongoose.Types.ObjectId
                (req.params.id)}},
