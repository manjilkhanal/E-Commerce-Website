...
export const DataTable = ({data, searchable = [], sortable = []}) => {
    ...
    useEffect(() => {
        ...
        for(let i = 1; i<= totalPages; i++){
            temp.push(<Pagination.Item active={i == currentNo} onClick={() =>
            setCurrentNo(i)}>{i}</Pagination.Item>)
        }

        temp.push(<Pagination.Prev  onClick={() => setCurrentNo(currentNo + 1)} disabled={currentNo == totalPages}/>)
    ...
    return <Row>
        ...
        <Table bordered striped hover size="sm">
            <thead className="table-dark">
                <tr>
                    {Object.keys(paginated[0]).map((key,i) =><th key=
                    {i} className={sortable.includes(key) ? 'sortable' :''} onClick={() =>{
