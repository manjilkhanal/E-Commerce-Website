    ...
    return <Container>
        <Row>
            <Col xs={12} className="bg-white my-3 py-3 rounded-2 shadow-sm">
                ...
                {loading ? <Loading/> : <Row>
                    <Col>
                        <DataTable searchable={['Name','Category','Brand','Price','Disc. Price','Featured','Status','Created At','Updated At']} sortable={['Name','Category','Brand','Price','Disc. Price','Featured','Status','Created At','Updated At']} data={products.map(product =>{
                            return {
                                'Name': product.name,
                                'Image': <a href={imgUrl(product.images[0])} target="_blank"><img src={imgUrl(product.images[0])}
                                className="img-sm"/></a>,
